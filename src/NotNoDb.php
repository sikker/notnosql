...
class NotNoDb {
	...
	private function createStatement() {
        if (!isset($this->preparedStatements['create'])) {
            $this->preparedStatements['create'] = $this->pdo->prepare('CREATE TABLE IF NOT EXISTS `' . $this->collectionTable . '` (`key` TEXT, `value` TEXT)');
		}
	...
	private function selectStatement(string $key) {
		if (!isset($this->preparedStatements['select'])) {
			$this->preparedStatements['select'] = $this->pdo->prepare('SELECT `value` FROM `' . $this->collectionTable . '` WHERE `key` = ?');
		}
	...
	private function insertStatement(string $key, $value) {
		if (!isset($this->preparedStatements['insert'])) {
			$this->preparedStatements['insert'] = $this->pdo->prepare('INSERT INTO `' . $this->collectionTable . '` (`key`, `value`) VALUES(?, ?)');
		}
	...
	private function updateStatement(string $key, $value) {
		if (!isset($this->preparedStatements['update'])) {
			$this->preparedStatements['update'] = $this->pdo->prepare('UPDATE `' . $this->collectionTable . '` SET `value` = ? WHERE `key` = ?');
		}
